...
gulp.task('clean:flag-icon', () =>
    del([`${COMPONENT_DIR}/flag-icon_size_*.css`, SPRITES_IMG_DIR, CHUNKS_IMG_DIR]));

SIZES.forEach((size) => {
    const cssChunksTemplate = hbs.compile(
        fs.readFileSync(`${COMPONENT_DIR}/flag-icon.chunks.css.hbs`).toString(),
    );
    const cssChunksData = { size, chunks: [] };

    // Process 2-char country code files only (ignore subregions)
    gulp.task(`flag-icon:resize-${size}`, () =>
        gulp
                ...
                gm((file, done) => {
                    file.size((error, dimensions) => {
                        if (error) {
                            ...
                            null,
                            optimizeGmFile(
                                file.resize(dimensions.width / 2, dimensions.height / 2),
                            ),
                        );
                    });
                }),
    ...
    gulp.task(`flag-icon:chunks-${size}`, [`flag-icon:resize-${size}`], () =>
        gulp
            .src(`${CHUNKS_IMG_DIR}/${size}/1x/*.png`)
            .pipe(
                gm((file, done) => {
                    file.size((error, dimensions) => {
                        if (error) {
    ...
    gulp.task(`flag-icon:sprite-${size}`, [`flag-icon:resize-${size}`], () => {
        const spriteData = gulp.src(`${CHUNKS_IMG_DIR}/${size}/**/*.png`).pipe(
            spritesmith({
        ...
        const cssStream = spriteData.css.pipe(gulp.dest(COMPONENT_DIR));
