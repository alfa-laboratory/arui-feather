...
class MaskedInput extends React.PureComponent<MaskedInputProps> {
    ...
    private processInputEvent(event) {
        ...
        if (prevSelection <= currentValue.length) {
            ...
            let opType =
                newValue.length >= currentValue.length ? operationType.ADD : operationType.DELETE;

            // На пользовательском инпуте было выделение перед операцией,
            // значит могла быть операция или удаления или замены.
            const beforeInputSelectionLength =
                this.beforeInputSelection.end - this.beforeInputSelection.start;
            ...
            const offsetSection =
                ...
                IS_ANDROID &&
                parseFloat(getAndroidVersion() as string) < 4.4
                    ? 1
                    : 0;
